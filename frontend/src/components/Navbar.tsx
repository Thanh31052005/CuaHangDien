...
export default function Navbar() {
  ...
  const categories = [
    { label: 'Đèn', slug: 'Đèn' },
    { label: 'Quạt', slug: 'Quạt' },
    { label: 'Pin', slug: 'Pin' },
    { label: 'Dây điện', slug: 'Dây điện' },
    { label: 'Ổ cắm & Công tắc', slug: 'Ổ cắm & Công tắc' },
    { label: 'Phụ kiện điện', slug: 'Phụ kiện điện' },
  ];
      ...
      <div className={`sticky top-0 z-50 bg-base-100 transition-shadow duration-300 ${scrolled ? 'shadow-md' : 'border-b border-base-200'}`}>
        <div className="container mx-auto px-4">
          <div className="flex items-center h-16 gap-4">
            {/* Logo */}
            <button onClick={() => navigate('home')} className="flex-shrink-0 flex items-center gap-2 group">
              ...
              <span className="font-black text-xl text-base-content">Elec<span className="text-primary">Pro</span></span>
            </button>
            ...
            <div className="dropdown hidden lg:block">
              ...
              <ul tabIndex={0} className="dropdown-content menu p-2 shadow-xl bg-base-100 border border-base-200 rounded-xl w-52 z-[100] mt-1">
                {categories.map(c => (
                  <li key={c.slug}>
                    <button onClick={() => { setSearchQuery(''); navigate('products', { category: c.slug }); }} className="hover:bg-primary/10 hover:text-primary rounded-lg font-medium">
                      {c.label}
                    </button>
                  </li>
                ))}
                  ...
                  <div className="absolute top-full left-0 right-0 bg-base-100 border border-base-200 rounded-xl shadow-xl z-[200] mt-1 overflow-hidden">
                    {suggestions.map(p => (
                      <button
                        ...
                        onClick={() => { navigate('product', { product: p }); setLocalQ(''); }}
                      >
            ...
            <div className="flex items-center gap-1">
              ...
              {/* Theme toggle */}
              <button onClick={toggleTheme} className="btn btn-ghost btn-circle btn-sm" title="Đổi giao diện">
                {theme === 'light' ? (
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <button onClick={() => setIsCartOpen(true)} className="btn btn-primary btn-sm gap-1.5 relative">
                ...
                {cartCount > 0 && (
                  <span className="badge badge-xs badge-warning absolute -top-2 -right-2 font-bold">{cartCount}</span>
                )}
              </button>
            </div>
          </div>
        ...
        {/* Category nav strip */}
        <div className="hidden lg:block border-t border-base-200 bg-base-50">
          <div className="container mx-auto px-4">
            <div className="flex items-center gap-1 overflow-x-auto scrollbar-none py-1.5">
              {categories.map(c => (
                <button
                  key={c.slug}
                  onClick={() => { setSearchQuery(''); navigate('products', { category: c.slug }); }}
              ...
              <div className="mt-2 flex flex-col gap-1">
                {suggestions.map(p => (
                  <button key={p.id} type="button"
                    className="flex items-center gap-3 p-2 hover:bg-base-200 rounded-xl text-left"
                    onClick={() => { navigate('product', { product: p }); setIsSearchOpen(false); setLocalQ(''); }}>
