        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          {cartItems.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full gap-4 text-base-content/50">
              ...
              <button onClick={() => { setIsCartOpen(false); navigate(ROUTES.PRODUCTS); }} className="btn btn-primary btn-sm">
                Tiếp tục mua sắm
              </button>
            </div>
          ) : (
        ...
        {cartItems.length > 0 && (
          <div className="border-t border-base-200 p-5 space-y-4">
            ...
            <div className="flex gap-2">
              <button onClick={() => { setIsCartOpen(false); navigate(ROUTES.CART); }} className="btn btn-outline flex-1">
                Xem giỏ hàng
              </button>
              <button onClick={() => { setIsCartOpen(false); navigate(ROUTES.CHECKOUT); }} className="btn btn-primary flex-1">
                Thanh toán
              </button>
            </div>
          </div>
