...
export default function LoginModal() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    login({ name: 'Nguyễn Văn A', email: 'user@elecpro.vn', role: 'user' });
  };

  const handleRegister = (e: React.FormEvent) => {
    e.preventDefault();
    login({ name: 'Khách hàng mới', email: 'new@elecpro.vn', role: 'user' });
  };
          ...
          {tab === 'login' ? (
            <form className="space-y-4" onSubmit={handleLogin}>
              ...
              <div className="flex flex-col gap-2">
                ...
                <a className="text-xs text-primary hover:underline self-end cursor-pointer">Quên mật khẩu?</a>
              </div>
